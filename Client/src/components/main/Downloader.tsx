  ...
  return (
    <>
      <div className="flex flex-col items-center text-center justify-center my-10 font-bold">
        <h1 className="text-xl md:text-4xl ">YouTube : Converter</h1>
        <h1 className="">
      ...
      <main className="box-holder max-w-[350px] md:max-w-[600px] lg:max-w-[900px] py-10 duratidon-200 flex flex-col gap-3">
        ...
        <div>
          <div className="bg-300 w-[300px] md:w-[450px] lg:w-[700px] rounded-lg justify-center py-4 px-4">
            {searchLoading ? (
              ...
              <div className="md:flex md:flex-wrap grid grid-cols-2 justify-center gap-4 p-2">
                {searchData?.links &&
                  Object.entries(searchData?.links?.mp4)?.map((l, i) => (
                    <div
                      className=" flex items-center justify-between lg:w-[160px] 
                      bg-100 hover:bg-white rounded-md cursor-pointer py-1 px-3"
                      key={i}
                      onClick={() => makeAjaxPostRequest(l[1]?.k)}
                    >
                      <div>
                        <p>{l[1]?.k}</p>
                        <small className="text-zinc-300">{l[1]?.size}</small>
                      </div>
                      {loading?.is && loading?.key == l[1]?.k ? (
                        ...
                        <span aria-label="Get link" className="relative tool">
